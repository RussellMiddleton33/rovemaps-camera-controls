...
// Z-up: Rotate GridHelper 90° around X-axis to align with XY ground plane
...
// Z-up: Show X (red) and Y (green) on ground plane, Z (blue) pointing up
const worldXArrow = makeArrow(new THREE.Vector3(1, 0, 0), 0xff0000);
const worldYArrow = makeArrow(new THREE.Vector3(0, 1, 0), 0x00ff00);
const camRightArrow = makeArrow(new THREE.Vector3(1, 0, 0), 0xff00ff); // magenta
const camFwdArrow = makeArrow(new THREE.Vector3(0, 1, 0), 0xffaa00);   // orange
...
for (let i = 0; i < 30; i++) {
  ...
  // Z-up: Objects sit on XY plane (z=0), positioned at x, y, with height 10 in Z
  mesh.position.set((Math.random() - 0.5) * 500, (Math.random() - 0.5) * 500, 10);
  scene.add(mesh);
}

const light = new THREE.DirectionalLight(0xffffff, 1);
// Z-up: Light position with positive Z for height
...
function buildController() {
  ...
  controller = new CameraController({
    ...
    upAxis: 'z', // Z-up coordinate system
...
function updateOverlay() {
  ...
  // Update gizmos at center (Z-up: use XY plane, z=0)
  const center3 = new THREE.Vector3(c.x, c.y, 0);
  ...
  // Z-up: bearing rotates in XY plane
  const right = new THREE.Vector3(Math.cos(br), Math.sin(br), 0);
  const fwd = new THREE.Vector3(-Math.sin(br), Math.cos(br), 0);
...
// Z-up: ground point is on XY plane (z=0)
let lastCenter = new THREE.Vector3(controller.getCenter().x, controller.getCenter().y, 0);
let lastTime = performance.now();
canvas.addEventListener('pointermove', (e) => {
  const rect = canvas.getBoundingClientRect();
  const pt = { x: e.clientX - rect.left, y: e.clientY - rect.top };
  const gp = (controller.transform as any).groundFromScreen?.(pt) ?? null;
  // Z-up: gx and gz in transform are actually gx and gy in Z-up (ground is XY plane)
...
controller.on('renderFrame', () => {
  ...
  lastTime = now;
  const c = controller.getCenter();
  const curr = new THREE.Vector3(c.x, c.y, 0);
