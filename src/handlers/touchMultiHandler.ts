...
export class TouchMultiHandler {
  ...
  private onMove = (e: PointerEvent) => {
    ...
    if (this.mode === 'pan' && this.opts.enablePan) {
      ...
      const world = ptr ? this.transform.screenToWorld(ptr) : null;
      ...
      if (ptr && world) { const sp = this.transform.worldToScreen(world); if (sp) { const cdx = sp.x - ptr.x; const cdy = sp.y - ptr.y; this.helper.handleMapControlsPan(this.transform, cdx, cdy); } }
